/*
  # Fix M-Pesa Integration Schema

  1. Changes
    - Drop any existing mpesa-related tables to ensure clean state
    - Create mpesa_configs table for API configuration
    - Create mpesa_transactions table with correct relationships
    - Add appropriate indexes for performance
    
  2. Security
    - Enable RLS
    - Add policies for users and admins
*/

-- Drop existing tables if they exist
DROP TABLE IF EXISTS mpesa_transactions CASCADE;
DROP TABLE IF EXISTS mpesa_configs CASCADE;

-- Create mpesa_configs table
CREATE TABLE IF NOT EXISTS mpesa_configs (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  business_shortcode text NOT NULL,
  passkey text NOT NULL,
  consumer_key text NOT NULL,
  consumer_secret text NOT NULL,
  environment text NOT NULL DEFAULT 'sandbox',
  created_at timestamptz DEFAULT now(),
  updated_at timestamptz DEFAULT now()
);

-- Create mpesa_transactions table
CREATE TABLE IF NOT EXISTS mpesa_transactions (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  order_id uuid REFERENCES orders(id) ON DELETE CASCADE,
  user_id uuid REFERENCES profiles(id) ON DELETE CASCADE,
  checkout_request_id text,
  merchant_request_id text,
  phone_number text NOT NULL,
  amount decimal(10,2) NOT NULL CHECK (amount > 0),
  reference_number text NOT NULL,
  description text,
  status text NOT NULL DEFAULT 'pending' CHECK (status IN ('pending', 'processing', 'completed', 'failed')),
  result_code text,
  result_desc text,
  created_at timestamptz DEFAULT now(),
  updated_at timestamptz DEFAULT now()
);

-- Add indexes for better query performance
CREATE INDEX IF NOT EXISTS mpesa_transactions_order_id_idx ON mpesa_transactions(order_id);
CREATE INDEX IF NOT EXISTS mpesa_transactions_user_id_idx ON mpesa_transactions(user_id);
CREATE INDEX IF NOT EXISTS mpesa_transactions_status_idx ON mpesa_transactions(status);
CREATE INDEX IF NOT EXISTS mpesa_transactions_created_at_idx ON mpesa_transactions(created_at DESC);

-- Enable RLS
ALTER TABLE mpesa_configs ENABLE ROW LEVEL SECURITY;
ALTER TABLE mpesa_transactions ENABLE ROW LEVEL SECURITY;

-- Policies for mpesa_configs
DO $$ 
BEGIN
  DROP POLICY IF EXISTS "Only admins can manage mpesa_configs" ON mpesa_configs;
  CREATE POLICY "Only admins can manage mpesa_configs"
    ON mpesa_configs FOR ALL
    TO authenticated
    USING (EXISTS (
      SELECT 1 FROM profiles WHERE id = auth.uid() AND role = 'admin'
    ));
END $$;

-- Policies for mpesa_transactions
DO $$ 
BEGIN
  DROP POLICY IF EXISTS "Users can view own transactions" ON mpesa_transactions;
  CREATE POLICY "Users can view own transactions"
    ON mpesa_transactions FOR SELECT
    TO authenticated
    USING (
      user_id = auth.uid() 
      OR EXISTS (
        SELECT 1 
        FROM profiles 
        WHERE id = auth.uid() AND role = 'admin'
      )
    );

  DROP POLICY IF EXISTS "Edge functions can create transactions" ON mpesa_transactions;
  CREATE POLICY "Edge functions can create transactions"
    ON mpesa_transactions FOR INSERT
    TO authenticated
    WITH CHECK (
      user_id = auth.uid()
      OR EXISTS (
        SELECT 1 
        FROM profiles 
        WHERE id = auth.uid() AND role = 'admin'
      )
    );

  DROP POLICY IF EXISTS "System can update transactions" ON mpesa_transactions;
  CREATE POLICY "System can update transactions"
    ON mpesa_transactions FOR UPDATE
    TO authenticated
    USING (
      user_id = auth.uid()
      OR EXISTS (
        SELECT 1 
        FROM profiles 
        WHERE id = auth.uid() AND role = 'admin'
      )
    );
END $$;